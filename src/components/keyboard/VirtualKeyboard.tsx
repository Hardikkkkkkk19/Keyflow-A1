...
export const VirtualKeyboard: React.FC<VirtualKeyboardProps> = ({
  ...
  const getKeyHeatColor = (keyChar: string) => {
    ...
    if (heatmapMetric === "accuracy") {
      if (match.accuracy < 90)
        return "bg-[#F05A9D]/90 text-white border-[#F05A9D] shadow-[#F05A9D]/30";
      if (match.accuracy < 95)
        return "bg-[#F4D35E]/90 text-[#0A0A0A] border-[#F4D35E] shadow-[#F4D35E]/20";
      return "bg-[#79D88B]/90 text-[#0A0A0A] border-[#79D88B] shadow-[#79D88B]/30";
      ...
      if (match.presses > 100)
        return "bg-[#45D6E8]/90 text-[#0A0A0A] border-[#45D6E8] shadow-[#45D6E8]/30";
      if (match.presses > 40) return "bg-[#45D6E8]/70 text-[#0A0A0A] border-[#45D6E8]/50";
      return "bg-[#181818] text-[#A0A0A0] border-[#262626]";
    ...
    return "bg-[#18C69A]/80 text-[#0A0A0A] border-[#18C69A]";
  };
  ...
  const containerStyles: Record<KeyboardStyle, string> = {
    classic: "bg-[#151515] rounded-2xl border border-[#262626] shadow-xl",
    cyber:
      "bg-[#0A0A0A] rounded-2xl border-2 border-[#18C69A]/50 shadow-[0_0_40px_rgba(24,198,154,0.25)] relative overflow-hidden",
    aurora:
      "bg-[#0D0D0D]/70 backdrop-blur-2xl rounded-3xl border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.6),inset_0_1px_1px_rgba(255,255,255,0.4)] relative overflow-hidden bg-gradient-to-br from-[#18C69A]/15 via-transparent to-[#45D6E8]/20",
    mechanical:
      "bg-[#151515] rounded-2xl border-2 border-[#262626] shadow-[0_24px_50px_rgba(0,0,0,0.9),0_0_0_1px_rgba(255,255,255,0.05)] p-4 sm:p-6 relative",
  };
  ...
  const getKeyStyle = (
    ...
    if (style === "cyber") {
      if (isPressed) {
        return "bg-[#45D6E8] text-[#0A0A0A] border-[#45D6E8] font-black scale-[0.95] translate-y-0.5 shadow-[0_0_30px_rgba(69,214,232,1)]";
      }
      if (isActive) {
        return "bg-[#181818] text-[#45D6E8] border-[#45D6E8] shadow-[0_0_20px_rgba(69,214,232,0.8),inset_0_0_10px_rgba(69,214,232,0.4)] font-extrabold animate-pulse";
      }
      let base =
        "bg-[#111111] hover:bg-[#181818] text-[#45D6E8] border-[#262626] hover:border-[#45D6E8] shadow-[0_0_8px_rgba(69,214,232,0.15)] font-mono";
      if (showFingerGuide && fingerInfo) {
        base += " border-l-2 border-l-[#45D6E8]";
      }
      return base;
    }
    ...
    if (style === "aurora") {
      if (isPressed) {
        return "bg-[#18C69A]/85 backdrop-blur-2xl text-[#0A0A0A] border-white font-black scale-[0.95] translate-y-0.5 shadow-[0_0_35px_rgba(24,198,154,0.9),inset_0_2px_4px_rgba(255,255,255,0.95)]";
      }
      if (isActive) {
        return "bg-[#18C69A]/30 backdrop-blur-2xl text-[#F5F5F5] border-[#18C69A] shadow-[0_0_25px_rgba(24,198,154,0.6),inset_0_1px_2px_rgba(255,255,255,0.8)] font-extrabold";
      }
      let base =
    ...
    if (style === "mechanical") {
      if (isPressed) {
        return "bg-gradient-to-b from-[#18C69A] to-[#14A782] text-[#0A0A0A] border-t border-t-white/90 border-b-0 font-black translate-y-[4px] shadow-[0_1px_0_0_#0A0A0A,inset_0_2px_6px_rgba(0,0,0,0.7)]";
      }
      if (isActive) {
        return "bg-gradient-to-b from-[#1C1C1C] to-[#151515] text-[#18C69A] border-t border-t-[#18C69A]/70 border-x border-x-[#18C69A]/40 border-b-4 border-b-[#0A0A0A] shadow-[0_5px_0_0_#0A0A0A,0_0_20px_rgba(24,198,154,0.5)] font-extrabold";
      }
      let base =
        "bg-gradient-to-b from-[#1C1C1C] to-[#151515] hover:from-[#262626] hover:to-[#181818] text-[#F5F5F5] border-t border-t-white/25 border-x border-x-white/10 border-b-4 border-b-[#0A0A0A] shadow-[0_5px_0_0_#0A0A0A,0_8px_16px_rgba(0,0,0,0.75)] font-bold transition-all duration-75";
      if (showFingerGuide && fingerInfo) {
        base += " border-l-2 border-l-[#18C69A]";
      }
      return base;
    ...
    // Default: Classic
    if (isPressed) {
      return "bg-[#18C69A] text-[#0A0A0A] border-[#18C69A] font-bold scale-[0.98] translate-y-0.5 shadow-inner";
    }
    if (isActive) {
      return "bg-[#181818] text-[#18C69A] border-[#18C69A]/60 shadow-xs font-bold";
    }
    let base = "bg-[#181818] hover:bg-[#1C1C1C] text-[#F5F5F5] border-[#262626] shadow-xs";
    if (showFingerGuide && fingerInfo) {
  ...
  return (
    ...
    >
      ...
      {showStyleSelector && (
        <div className="flex flex-wrap items-center justify-between mb-3.5 pb-2.5 border-b border-[#262626] gap-2">
          ...
          <div className="flex items-center gap-1 bg-[#111111] p-1 rounded-xl border border-[#262626] overflow-x-auto">
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all cursor-pointer flex items-center gap-1.5 whitespace-nowrap ${
                    isSelected
                      ? "bg-[#18C69A] text-[#0A0A0A] font-bold shadow-xs"
      ...
      {currentStyle === "cyber" && (
        <>
          <div className="absolute top-0 inset-x-0 h-0.5 bg-gradient-to-r from-transparent via-[#45D6E8] to-transparent shadow-[0_0_12px_#45D6E8]" />
      ...
      {currentStyle === "aurora" && (
        <>
          ...
          <div className="absolute -bottom-12 -right-12 w-48 h-48 rounded-full bg-[#45D6E8]/25 blur-3xl pointer-events-none" />
        </>
      )}
      ...
      {isHeatmapMode && (
        <div className="mb-3 px-3 py-1.5 rounded-lg bg-[#111111] border border-[#262626] text-xs font-mono text-[#A0A0A0] flex items-center justify-between min-h-[32px]">
          {hoveredKey ? (
            <div className="flex flex-wrap items-center gap-4 text-[11px]">
              <span className="font-bold text-[#0A0A0A] text-sm bg-[#18C69A] px-2 py-0.5 rounded">
                Key '{hoveredKey.displayLabel}'
              </span>
              <span>
                Acc: <strong className="text-[#79D88B]">{hoveredKey.accuracy}%</strong>
              </span>
              <span>
                Errors: <strong className="text-[#F05A9D]">{hoveredKey.errors}</strong>
              </span>
              <span>
                Usage: <strong className="text-[#F5F5F5]">{hoveredKey.presses}</strong>
              </span>
              <span>
                Avg Latency: <strong className="text-[#F4D35E]">{hoveredKey.avgLatencyMs}ms</strong>
              </span>
            </div>
          ) : (
            ...
            {row.map((key) => {
              ...
              return (
                ...
                >
                  ...
                  {key.subLabel && (
                    <span className="text-[9px] sm:text-[10px] text-[#666666] font-mono leading-none z-10">
                      {key.subLabel}
                    </span>
                  )}
      ...
      <div className="mt-4 pt-3 border-t border-[#262626] flex items-center justify-between text-[11px] text-[#A0A0A0] px-2">
        <div className="flex items-center gap-4">
          {showFingerGuide && (
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#18C69A]" />
              <span>Left Hand</span>
              <span className="w-2 h-2 rounded-full bg-[#18C69A] ml-2" />
          ...
          {isHeatmapMode && (
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-[#79D88B]" /> High Performance
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-[#F4D35E]" /> Medium Performance
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-[#F05A9D]" /> Needs Practice
              </span>
            </div>
          )}
        </div>
        <div className="text-[#666666] font-mono hidden sm:block">
