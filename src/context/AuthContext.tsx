...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Ref to track in-flight or completed challenge IDs to prevent duplicate claim operations
  const processingChallengeIdsRef = useRef<Set<string>>(new Set());
  ...
  const mergeProfileProgression = (
    ...
    // Also merge challenges from Supabase Auth user_metadata and dedicated key
    if (Array.isArray(currentUser.user_metadata?.completed_challenges)) {
      for (const chId of currentUser.user_metadata.completed_challenges) {
        completedChallengesSet.add(chId);
      }
    }
    try {
      const raw = localStorage.getItem(`keyflow_completed_challenges_${currentUser.id}`);
      if (raw) {
        const arr = JSON.parse(raw);
        if (Array.isArray(arr)) arr.forEach((id) => completedChallengesSet.add(id));
      }
    } catch {
      // Ignore parse failure on corrupted storage key
    }
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        if (data) {
          ...
          const levelInfo = calculateLevelInfo(dbXp);
          const dbHighestWpm = data.highest_wpm ?? 0;
          const dbAvgWpm = data.avg_wpm ?? 0;
          const dbAvgAcc = data.avg_accuracy ?? 0;
          const dbTotalSessions = data.total_sessions ?? 0;
          const dbStreak = data.streak_days ?? 0;
          const name = data.display_name || displayName;
          const handle = `@${name.toLowerCase().replace(/[^a-z0-9_]/g, "")}`;

          // Retrieve cached stats & challenges for this user
          const existingLocal = loadUserStats(currentUser.id, email, name);

          // Get challenge completion list from:
          // 1. Supabase Auth user_metadata.completed_challenges
          // 2. localStorage keyflow_completed_challenges_<userId>
          // 3. existingLocal.completedChallenges
          const authChallenges: string[] = Array.isArray(
            currentUser.user_metadata?.completed_challenges,
          )
            ? currentUser.user_metadata.completed_challenges
            : [];
          const localChallenges: string[] = Array.isArray(existingLocal.completedChallenges)
            ? existingLocal.completedChallenges
            : [];
          const dedicatedStorageChallenges: string[] = (() => {
            try {
              const raw = localStorage.getItem(`keyflow_completed_challenges_${currentUser.id}`);
          ...
          const mergedChallenges = Array.from(
            new Set([...authChallenges, ...localChallenges, ...dedicatedStorageChallenges]),
          );

          // Sync back to dedicated storage key and Auth metadata if needed
          localStorage.setItem(
            `keyflow_completed_challenges_${currentUser.id}`,
          ...
          if (
            ...
            mergedChallenges.length !== authChallenges.length
          ) {
            supabase.auth
              .updateUser({
                data: { completed_challenges: mergedChallenges },
              })
              .catch(() => {});
          }
          ...
          const freshStats: UserProfile = {
            ...
            name,
            handle,
            level: data.level ?? levelInfo.level ?? 1,
            currentXp: Math.max(dbXp, existingLocal.currentXp || 0),
            nextLevelXp: levelInfo.xpForNextLevelTotal,
            streakDays: Math.max(dbStreak, existingLocal.streakDays || 0),
            highestWpm: Math.max(dbHighestWpm, existingLocal.highestWpm || 0),
            avgWpm: dbAvgWpm || existingLocal.avgWpm || 0,
            avgAccuracy: dbAvgAcc || existingLocal.avgAccuracy || 0,
            totalTestsCompleted: Math.max(dbTotalSessions, existingLocal.totalTestsCompleted || 0),
            completedChallenges: mergedChallenges,
          };

          setUserStatsProfile(freshStats);
          saveUserStats(currentUser.id, freshStats);
  ...
  const signOut = async (): Promise<void> => {
    ...
    processingChallengeIdsRef.current.clear();
  };
  ...
  const recordChallengeCompletion = async (
    challengeId: string,
    challengeTitle: string,
    rewardXp: number,
  ) => {
    ...
    // Idempotency: verify ref and existing completed challenges list
    if (processingChallengeIdsRef.current.has(challengeId)) {
      return;
    }
    ...
    if (completedList.includes(challengeId)) {
      return; // Already completed and claimed
    }

    // Mark as processing to prevent rapid double-clicks
    processingChallengeIdsRef.current.add(challengeId);

    const updatedCompleted = Array.from(new Set([...completedList, challengeId]));
    ...
    const cumulativeXp = (prev.currentXp || 0) + rewardXp;
    ...
    saveUserStats(user.id, updated);
    localStorage.setItem(
      `keyflow_completed_challenges_${user.id}`,
      JSON.stringify(updatedCompleted),
    );

    // Persist to Supabase database (keyflow_users)
    await persistUserStatsToSupabase(user.id, updated);

    // Persist to Supabase Auth metadata for persistent cross-device storage
    if (isSupabaseConfigured) {
      try {
        await supabase.auth.updateUser({
          data: { completed_challenges: updatedCompleted },
        });
      } catch (err) {
        console.warn("Supabase auth user_metadata challenge persist notice:", err);
      }
    }
